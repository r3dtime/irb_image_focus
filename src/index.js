const links = (selector, props) => {
    ...
    const inPercent = (v, t) => (v / t) * 100;
    const inNumber = (p, t) => ((p > 100 ? 100 : p) / 100) * t;
    const insideContainment = (x, y, c) => !(x < (c.x) || x > (c.x + c.width) || y < (c.y) || y > (c.y + c.height));
    ...
    document.querySelectorAll(selector).forEach((w) => {
        ...
        let img = w.getBoundingClientRect();
        ...
        ['opacity', 'width', 'height', 'padding', 'margin'].map((k) => inputPos.style[k] = 0);
        ...
        const handleDotMove = (e) => {
            if (!isDown) { return; }
            if (!insideContainment(e.clientX, e.clientY, w.getBoundingClientRect())) { return; }
            updatePosition(e)
            updateDot()
        }
        focus.addEventListener('mousedown', (e) => {
            img = w.getBoundingClientRect();
            if (!insideContainment(e.clientX, e.clientY, img)) { return; }
            isDown = true;
            updatePosition(e)
            updateDot()
        }, true);
        ...
        */
        focus.addEventListener('mouseup', (e) => {
            isDown = false;
        ...
        loadingBtn.onclick = () => { input.click(); };
        ...
        input.accept = props.accept.map((e) => 'image/' + e).join(', ');
        ['opacity', 'width', 'height', 'padding', 'margin'].map((k) => input.style[k] = 0);
        input.onchange = (e) => {
